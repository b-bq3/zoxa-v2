#!/usr/bin/env python3
# V3 — ZOXA full name in center, professional, no solo Z
...
def save(s, url, ext):
    ...
    p = os.path.join(dusk_cfg['save'], f'zoxa_v3{ext}')
    with open(p, 'wb') as f:
        f.write(r.content)
...
# V3 prompt: full "ZOXA" name dominates the icon
prompt = """Professional PWA app icon for a website called "Zoxa".
512x512 square, dark background #0f0f19, rounded corners.
The word "ZOXA" in LARGE bold letters dominates the entire center of the icon — the name is the main visual.
Deep red gradient text (#cc0000 to #8b0000), elegant modern font.
Subtle red glow behind the text. Thin red border glow on edge.
Minimalist, clean, modern. No pixel art. No Minecraft. No pickaxe. No tools. No stars. No decorations.
This is a tech brand icon, not a game icon.
